...
def get_macro_category(cat):
    ...
    if cat in ['🥃 Виски', '💧 Водка', '🏴‍☠️ Ром', '🌵 Текила', '🌲 Джин', '🍇 Коньяк/Бренди', '🍒 Ликер/Настойка']: 
        return '🥃 Крепкое'
    # Еда, Вино, Коктейли и Допы остаются как есть
    return cat

# --- 2. ГРАНУЛЯРНЫЙ КАТЕГОРИЗАТОР С ВШИТОЙ БАЗОЙ (МИКРО-УРОВЕНЬ) ---
...
if st.session_state.df_full is not None:
    # Динамически добавляем Макро-категории
    ...
    with tab2:
        ...
        with c2:
            ...
            # Группируем блюда, но показываем выбранный уровень категории
            ...
            # Переименовываем колонку для красоты в таблице
            df_menu = df_menu.rename(columns={target_cat: 'Категория'})
            
            st.dataframe(
                df_menu[['Блюдо', 'Категория', 'Выручка с НДС', 'Фудкост %']].style
