...
# --- ГРАНУЛЯРНЫЙ КАТЕГОРИЗАТОР (V19.0) ---
def detect_category_granular(name_input):
    ...
    # Пиво Розлив (Остальное пиво считаем розливом)
    ...
    # 4.8 Ликеры и Настойки (все остальное крепкое сладкое)
...
def process_single_file(file_content, filename=""):
    try:
        ...
        # ПРИМЕНЯЕМ НОВЫЙ ГРАНУЛЯРНЫЙ КАТЕГОРИЗАТОР
...
# --- МЕНЕДЖЕР КАТЕГОРИЙ (ОСТАВЛЯЕМ ДЛЯ РУЧНОЙ ПРАВКИ) ---
st.sidebar.write("---")
...
if st.session_state.df_full is not None:
    ...
    with tab2:
        ...
        with c1:
            ...
            # Группировка по НОВЫМ категориям
