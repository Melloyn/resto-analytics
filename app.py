...
# --- УМНЫЙ КАТЕГОРИЗАТОР (НА ОСНОВЕ ТВОЕЙ МАТРИЦЫ) ---
def detect_category(name_input):
    name = str(name_input).lower()
    
    # 1. Сначала ищем по жестким брендам из твоего файла
    ...
    # КРЕПКОЕ (Водка, Виски, Джин, Ром, Текила, Коньяк, Настойки)
    strong_brands = [
    ...
    # КОКТЕЙЛИ (Спец. названия из контракта)
    cocktail_brands = [
    ...
    # ЕДА (Старый список, т.к. в файле нет еды)
    ...
    # 1. Проверяем точные совпадения брендов из матрицы
    ...
    # 2. Если бренд не найден (новинка), используем старые общие слова (подстраховка)
...
def process_single_file(file_content, filename=""):
    try:
        ...
        # Фильтруем строки, которые совпадают с нашими тех. именами или категориями
...
if st.session_state.df_full is not None:
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["🔥 Инфляция и Потери", "🍰 Меню и Косты", "⭐ Матрица (ABC)", "🗓 Дни недели"])
